...
def form_aurora_batch(df: pd.DataFrame):
    ...
    print(f"最终选的4个站点: {available_stations}")
    ...
    if len(station_columns) != 4:
        print(f"没4个站点，只有{len(station_columns)}个")
    ...
    try:
        ...
        if actual_shape[1] != expected_shape[1]:
            raise ValueError(f"数据形状不匹配，应该是{expected_shape}, 现在是{actual_shape}")
        
        # 重新整成 (t, features, stations) 然后再2x2
        ...
        for i in [0]:  # 就按第一个点检查
            ...
            temps = [
                data_array[i, temp_idx, 0, 0],  # (0,0)
                data_array[i, temp_idx, 0, 1],  # (0,1) 
                data_array[i, temp_idx, 1, 0],  # (1,0)
                data_array[i, temp_idx, 1, 1]   # (1,1)
            ]
    ...
    for i in range(1, t-2):
        ...
        if i == 1:
            ...
            print(f"实际顺序: {feature_order.tolist()}")
    ...
    print(f"最后有 {len(X_batches)} 个批次生成")
