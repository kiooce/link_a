...
def create_october_filtered_visualization(all_preds, all_labels, X_batch, y_batch, df_original):
    ...
    for i, X in enumerate(X_batch):
        ...
        elif input_time.month == 10:
            filtered_count += 1
    
    if len(october_indices) == 0:
        print("没有符合条件的数据可以可视化")
    ...
    print(f"10月份2小时间隔数据形状: {october_preds.shape}")
    print(f"筛选掉了 {filtered_count} 个非2小时间隔的10月数据")
...
def main(model, X_batch, y_batch, device, df_original):
    ...
    print("=== 紧急验证：检查站点数据对应关系 ===")

    test_batch_idx = 0
    if test_batch_idx < len(X_batch):
        X_test = X_batch[test_batch_idx]
        input_time = X_test.metadata.time[0]
        
        # 检查原始数据
        original_data = df_original[df_original['time'] == input_time]
        print(f"测试时间: {input_time}")
        print("原始数据各站点温度:")
        for _, row in original_data.iterrows():
            print(f"  {row['station_name']}: {row['temperature']:.2f}K")
        
        # 检查输入批次的2x2温度
        input_temp_2x2 = X_test.surf_vars['2t'][0, 1]  # 第一个时间步
        print("输入批次2x2温度网格:")
        print(f"  (0,0): {input_temp_2x2[0,0]:.2f}K  (0,1): {input_temp_2x2[0,1]:.2f}K")
        print(f"  (1,0): {input_temp_2x2[1,0]:.2f}K  (1,1): {input_temp_2x2[1,1]:.2f}K")
        
        # 检查预测结果的144点分段
        if test_batch_idx < len(october_preds):
            pred_temp_144 = october_preds[test_batch_idx, 0]  # 温度预测
            print("预测结果144点分段温度均值:")
            print(f"  上清寺(0-35): {pred_temp_144[0:36].mean():.2f}K")
            print(f"  唐家沱(36-71): {pred_temp_144[36:72].mean():.2f}K") 
            print(f"  天生(72-107): {pred_temp_144[72:108].mean():.2f}K")
            print(f"  龙井湾(108-143): {pred_temp_144[108:144].mean():.2f}K")
            
            # 关键验证：上清寺原始数据 vs Excel输出数据
            shangqingsi_original = original_data[original_data['station_name'] == '上清寺']
            if len(shangqingsi_original) > 0:
                orig_temp = shangqingsi_original.iloc[0]['temperature']
                excel_temp = pred_temp_144[0:36].mean().cpu().numpy()
                print(f"\n关键验证 - 上清寺:")
                print(f"  原始温度: {orig_temp:.2f}K")
                print(f"  Excel输出预测: {excel_temp:.2f}K")
                print(f"  2x2网格(0,0): {input_temp_2x2[0,0]:.2f}K")
                
                # 检查是否对应
                if abs(orig_temp - input_temp_2x2[0,0]) < 0.01:
                    print("  ✓ 原始数据与2x2网格(0,0)匹配")
                else:
                    print("  ✗ 原始数据与2x2网格(0,0)不匹配，可能有映射错误!")
...
def export_shangqingsi_predictions_to_excel(preds, labels, timestamps, X_batch, y_batch, df_original):
    ...
    for batch_idx in range(len(preds)):
        ...
        # 再次验证时间间隔（双重检查）
        if not is_valid_2hour_interval(input_time, pred_time):
        ...
        row_data_processed = {
            ...
            '预测间隔(小时)': round(time_diff_hours, 2)  # 更精确显示
        }
        ...
        for var_name, var_idx in variables.items():
            ...
            row_data_processed[f'{clean_var_name}_error_percent'] = abs(float(pred_mean - true_mean)) / (abs(float(true_mean)) + 1e-8) * 100
        ...
        row_data_original.update({
            ...
            'pred_humidity_raw': float(np.clip(pred_humidity_percent, 0, 100)),  # 限制湿度范围
            'pred_pressure_raw': float(pred_pressure_processed),
            'pred_wind_speed_raw': float(max(0, pred_wind_speed)),  # 风速不能为负
            'pred_wind_dir_raw': float(pred_wind_dir % 360),  # 确保风向在0-360度
            'pred_pm25_raw': float(max(0, preds[batch_idx, 5, start:end].mean().cpu().numpy())),
            'pred_pm10_raw': float(max(0, preds[batch_idx, 4, start:end].mean().cpu().numpy())),
            'pred_so2_raw': float(max(0, preds[batch_idx, 6, start:end].mean().cpu().numpy())),
            'pred_no2_raw': float(max(0, preds[batch_idx, 7, start:end].mean().cpu().numpy())),
            'pred_o3_raw': float(max(0, preds[batch_idx, 8, start:end].mean().cpu().numpy())),
            'pred_co_raw': float(max(0, preds[batch_idx, 9, start:end].mean().cpu().numpy())),
        })
        ...
        if not np.isnan(original_pred.get('temperature_raw', np.nan)):
            ...
            row_data_original['temp_error_percent'] = row_data_original['temp_error_raw'] / (abs(original_pred['temperature_raw']) + 1e-8) * 100
        if not np.isnan(original_pred.get('pm25_raw', np.nan)):
            pred_pm25_val = float(max(0, preds[batch_idx, 5, start:end].mean().cpu().numpy()))
            row_data_original['pm25_error_raw'] = abs(pred_pm25_val - original_pred['pm25_raw'])
            row_data_original['pm25_error_percent'] = row_data_original['pm25_error_raw'] / (abs(original_pred['pm25_raw']) + 1e-8) * 100
    ...
    # 保存到excel
    filename_processed = '上清寺_10月份预测结果_预处理版_2小时.xlsx'
    filename_original = '上清寺_10月份预测结果_原始数据版_2小时.xlsx'
    ...
    print(f"筛选掉了 {invalid_interval_count} 个非2小时间隔的批次")
    
    # 详细的时间间隔统计
    if len(df_export_processed) > 0:
        time_diff_hours = []
        ...
        print(f"时间间隔统计: 最小{min(time_diff_hours):.2f}h, 最大{max(time_diff_hours):.2f}h, 平均{np.mean(time_diff_hours):.2f}h, 标准差{np.std(time_diff_hours):.3f}h")
